...
module.exports= {
    ...
    toComponentName: function(file, c, name){
        name= name ? "/"+name : "";
        const fn= this.toFileName(file), _fn= fn.substring(0, fn.indexOf("."));
        if(_fn==="inc") return fn.substring(fn.indexOf(".")+1, fn.lastIndexOf("."))+name;
        return _fn==="index"&&c ? c : (_fn+name);
    },
    ...
    /* API PART */
    isRequest: function(file, type){
        if(type==="function") return "funkce/metoda";
    ...
    defaultParams: function(template_main, template, type){
        if(!type) return "";
        const last_sync= [ "`SQL TIMESTAMP`", "Poslední synchronizace" ];
        const def= {
            user: {
        ...
        const text= type==="function" ? "použít jako argumenty funkce" : "použít jako **POST** klíče";
    ...
    defaultReturns: function(ret, template){
        ...
        return this.partial(template, { success: [ "`Number`", "Vrací `0` (neúspěch), `1` (**úspěch**), `2` (neúspěch s hláškou)." ], last_sync: [ "`SQL TIMESTAMP`", "Časové razítko synchronizace." ] }, "loop", "\n")+"\n";
    },
    /* BOTH */
    toStructure: function(id_dynamic_name, data){
        ...
        for(let i=0, map_i, map_i_file, map_i_link, map_i_arr, map_i_arr_l, map_i_arr_last;(map_i= map[i]); i++){
            ...
            if(map_i_file!=="main") map_i_arr.push(map_i_file);
            if(map_i.name) map_i_arr.push(map_i.name);
            ...
            for(let j=deep_l, map_ij;(map_ij= map_i_arr[j]); j++){
                ...
                if(j===map_i_arr_last){
                    description= ` *(${this[id_dynamic_name](map_i.params,map_i.depends)})*\n\n${indent}    `+this.handleLinks(this.joinLines(map_i.description, " "));
                    out+= `${indent}- **[${map_ij}](#nav_${map_i_link})**${map_i_file==="main" ? target : ""}${description}\n\n`;
                } else {
